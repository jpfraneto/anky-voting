...
async function processVotedCharacters() {
  console.log(
    'Inside the process voted characters function for the world number 7'
  );
  let index = 0;
  try {
    ...
    for (const character of characters) {
      ...
      fs.mkdirSync('CLARIDIUM', { recursive: true });

      // Create a file path for the new file
      const filePath = path.join('CLARIDIUM', `${character.tokenId}.png`);
